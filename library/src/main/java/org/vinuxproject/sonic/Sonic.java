...
public class Sonic {
    ...
    private int findPitchPeriodInRange(short samples[], int position, int minPeriod, int maxPeriod, Integer retMinDiff, Integer retMaxDiff) {
    ...
    private int findPitchPeriod(short samples[], int position, boolean preferNewPeriod) {
        ...
        if (numChannels == 1 && skip == 1) {
            ...
            period = findPitchPeriodInRange(downSampleBuffer, 0, minPeriod / skip, maxPeriod / skip, minDiff, maxDiff);
            if (skip != 1) {
                period *= skip;
    ...
    // Overlap two sound segments, ramp the volume of one down, while ramping the
    // other one from zero up, and add them, storing the result at the output.
    private void overlapAddWithSeparation(int numSamples, int numChannels, int separation, short out[], int outPos, short rampDown[], int rampDownPos,
    ...
    public static int changeFloatSpeed(float samples[], int numSamples, float speed, float pitch, float rate, float volume, boolean useChordPitch,
    ...
    /* This is a non-stream oriented interface to just change the speed of a sound sample */
    public int sonicChangeShortSpeed(short samples[], int numSamples, float speed, float pitch, float rate, float volume, boolean useChordPitch, int sampleRate,
            int numChannels) {
